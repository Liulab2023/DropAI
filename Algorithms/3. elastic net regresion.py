...
from sklearn.model_selection import GridSearchCV
param_grid = [{'alpha': np.linspace(0,6,601)}]
...
ElasticNet_model = ElasticNet(alpha= 0.79)
model = ElasticNet_model.fit(X_train,y_train)
trainpre = ElasticNet_model.predict(X_train)
...
ElasticNet_model = ElasticNet(alpha= 0.79)
model = ElasticNet_model.fit(X_train,y_train)
testpre = ElasticNet_model.predict(X_test)
